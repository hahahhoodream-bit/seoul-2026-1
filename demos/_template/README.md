...
1. **(기능 1) 데이터 업로드 및 현황 확인**: 다중 인코딩 자동 감지 기능으로 엑셀 출력 CSV 파일을 깨짐 없이 업로드하고, 학급 전체의 응답 완성도 메트릭과 데이터 표를 확인합니다.
2. **(기능 2) 관계도 네트워크 시각화**: `NetworkX`와 `Plotly`를 통해 학생 간 친밀(초록)/서먹(빨간) 관계를 화살표 그래프로 시각화하며, 노드 크기는 받은 선택량에 비례하고 호버 시 친밀/소외 세부 인원수가 쪼개져 나타납니다.
3. **(기능 3) 관계 분석 지표 및 사유 그룹 요약**: 친밀도 상위 5명과 소외도 상위 5명 명단을 추출하고, 파편화된 주관식 지목 사유들을 의미론적으로 결합하여 교사가 알아보기 쉬운 '문장 형태의 요약 결과'를 제공합니다.
