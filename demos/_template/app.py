...
# ──────────────────────────────────────────────────────────────
# 공용 유틸 및 검증 (초등교사 맞춤형 에러 메시지 고도화 반영)
# ──────────────────────────────────────────────────────────────
...
def validate_csv(df) -> tuple[bool, str]:
    """[★#10 고도화★] 교사가 이해하기 쉬운 친절한 한글 에러 메시지로 커스텀"""
    required_cols = ["name", "best_friend", "distant_friend", "reason_pos", "reason_neg"]
    
    # 1. 컬럼명 누락 검증
    if not all(col in df.columns for col in required_cols):
        missing = [c for c in required_cols if c not in df.columns]
        return False, f"엑셀 파일에 필수 열(컬럼)이 누락되었습니다. 제외된 열: {missing} | 파일 양식을 다시 확인해 주세요."
    
    # 2. 학생 수(행 수) 검증
    if len(df) != 22:
        return False, f"학급 학생 수 설정(22명)과 업로드된 데이터의 행 수가 일치하지 않습니다. (현재 업로드된 학생 수: {len(df)}명)"
    
    # 3. 빈 칸(공백) 검증
    for col in required_cols:
        if df[col].isna().any() or (df[col] == "").any():
            # 실제 엑셀 줄번호와 맞추기 위해 index에 +2 처리 (헤더 행 1줄 + 인덱스 0 시작 고려)
            ...
            return False, f"엑셀 파일의 [{col_korean}] 열에 빈 칸이 발견되었습니다. (해당 엑셀 줄번호: {missing_rows}번째 줄) 공백을 채운 뒤 다시 업로드해 주세요."
...
def visualize_network(G, df):
    """[★#9 고도화★] Plotly 그래프 시각화 및 호버 툴팁 정보 세분화"""
    pos = nx.spring_layout(G, k=2, iterations=50, seed=42)
    
    # 각 학생별 친밀/소외 선택 횟수 정밀 세분화 계산
    node_details = {}
    for name in df["name"]:
    ...
    # 호버(Hover) 시 세부 인원수가 쪼개져 보이도록 툴팁 텍스트 빌드업
    hover_texts = []
    for label in node_labels:
        d = node_details[label]
        hover_texts.append(f"<b>{label}</b><br>선택 받음: 총 {d['total']}명<br>  - 😊 친한 친구: {d['best']}명<br>  - 🔺 서먹한 친구: {d['distant']}명")
...
def summarize_reasons_by_group(reason_list, is_positive=True):
    ...
    for reason in reason_list:
        ...
        if is_positive:
            if any(w in text for w in ["성격", "착하다", "활발", "밝다", "좋다", "친하다"]): categories["성격이 좋고 활발함"] += 1
            elif any(w in text for w in ["친절", "도와", "배려", "잘해", "착해"]): categories["친절하고 배려심이 많음"] += 1
            elif any(w in text for w in ["재미", "웃기", "재밌", "개그"]): categories["유머러스하고 같이 있으면 즐거움"] += 1
            elif any(w in text for w in ["취미", "게임", "통한다", "관심사", "이야기", "코드"]): categories["취미나 대화가 잘 통함"] += 1
            else: categories["기타 친근감 표시"] += 1
        else:
            if any(w in text for w in ["말", "대화", "이야기", "서먹", "어색", "안친"]): categories["평소 대화나 소통이 부족함"] += 1
            elif any(w in text for w in ["싸움", "장난", "괴롭", "시비", "때려"]): categories["과도한 장난이나 갈등이 있었음"] += 1
            elif any(w in text for w in ["무시", "이기", "욕", "성격", "나빠"]): categories["성격적 차이 및 배려 부족"] += 1
            elif any(w in text for w in ["반", "조", "기회", "자리"]): categories["같은 모둠이나 놀 기회가 없었음"] += 1
            else: categories["특별한 갈등 없는 단순 서먹함"] += 1

    top_categories = categories.most_common(2)
    if not top_categories: return "특별한 사유 없음"
...
# ──────────────────────────────────────────────────────────────
# [기능 ③] 관계 분석 지표 & 사유 그룹 요약 연동
# ──────────────────────────────────────────────────────────────
st.subheader("③ 관계 분석 지표")
metrics_df = calculate_network_metrics(df, G)

with st.expander("📖 지표 설명 보기"):
    st.markdown("- **👍 받은 친밀표시 / 👎 받은 소외표시**: 다른 또래들이 나를 지목한 누적 횟수\n- **🔗 중심성**: 사교성과 교우관계의 활성화 정도\n- **📊 관계성향**: 데이터 기반 자동 분류 성향")
...
with col_pos:
    ...
    top_5_intimate["🎯 주된 친밀 사유 요약 (문장그룹)"] = [summarize_reasons_by_group(df[df["best_friend"] == r["이름"]]["reason_pos"].tolist(), is_positive=True) for _, r in top_5_intimate.iterrows()]
    st.dataframe(top_5_intimate[["이름", "👍 받은 친밀표시", "🎯 주된 친밀 사유 요약 (문장그룹)"]], use_container_width=True, hide_index=True)

with col_neg:
    st.markdown("### ⚠️ 우리반 소외도 상위 5명")
    top_5_isolated = metrics_df.nlargest(5, "👎 받은 소외표시").copy()
    top_5_isolated["🎯 주된 서먹한 사유 요약 (문장그룹)"] = [summarize_reasons_by_group(df[df["distant_friend"] == r["이름"]]["reason_neg"].tolist(), is_positive=False) for _, r in top_5_isolated.iterrows()]
    st.dataframe(top_5_isolated[["이름", "👎 받은 소외표시", "🎯 주된 서먹한 사유 요약 (문장그룹)"]], use_container_width=True, hide_index=True)
